# -*- coding: utf-8 -*-
"""
Created on Sat Apr 14 00:36:32 2018
@author: NIZI
"""
...
'''
###### Audio branch 1
# define left input and shape (XXX, 64)
left_input = Input(shape=(602, 64))
# define right input and shape (XXX, 64)
right_input = Input(shape=(602, 64))
# concatenate two channels
audio = concatenate([left_input, right_input], name='merge_layer')
# setup masking layer before LSTM layer
audio = Masking(mask_value=0.)(audio)
# LSTM layer
audio = LSTM(512,
             return_sequences=True,
             recurrent_dropout=0.25,
             name='LSTM_audio')(audio)
...
left_input = Input(shape=(602, 64))
left_input_mask = Masking(mask_value=0.)(left_input)
left_input_LSTM = LSTM(512,
                       return_sequences=True,
                       recurrent_dropout=0.25,
                       name='LSTM_left_audio')(left_input_mask)
left_audio_weight = AttentionLayer()(left_input_LSTM)
left_audio_weight = Lambda(weight_expand)(left_audio_weight)
left_audio_vector = Lambda(weight_dot)([left_input, left_audio_weight])
left_audio_feature_vector = Lambda(lambda x: backend.sum(x, axis=1))(left_audio_vector)

# calculate right audio feature vector
right_input = Input(shape=(602, 64))
right_input_mask = Masking(mask_value=0.)(right_input)
right_input_LSTM = LSTM(512,
                        return_sequences=True,
                        recurrent_dropout=0.25,
                        name='LSTM_right_audio')(right_input_mask)
right_audio_weight = AttentionLayer()(right_input_LSTM)
right_audio_weight = Lambda(weight_expand)(right_audio_weight)
right_audio_vector = Lambda(weight_dot)([right_input, right_audio_weight])
...
audio_prediction_0 = Dense(256, activation='softmax')(audio_feature_vector)
dropout_audio_0 = Dropout(0.5)(audio_prediction_0)
audio_prediction_1 = Dense(128, activation='softmax')(dropout_audio_0)
dropout_audio_1 = Dropout(0.5)(audio_prediction_1)
audio_prediction = Dense(numclass, activation='softmax')(dropout_audio_1)
...
# dropout layer
dropout_text = Dropout(0.5)(text_feature_vector)
...
if __name__ == "__main__":
    ...
    # print result
    print('>>>Training done.')
    print('>Text Acc:' + str(result_t))
    print('>Audio Acc:' + str(result_a))
